...
const show=()=>render(<ConvertToBypassModal lead={lead} sourceDocumentId="source" onClose={vi.fn()} onDone={vi.fn()} />);
beforeEach(()=>{mocks.update.mockReset().mockResolvedValue(undefined);mocks.read.mockReset();vi.stubGlobal('fetch',vi.fn().mockResolvedValue({ok:true,json:async()=>({fieldsFilled:2})}));});
