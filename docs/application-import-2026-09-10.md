...
Known limits: field mapping recognizes common English labels, not arbitrary partner schemas or every handwriting style. Conflicting values and malformed dates, amounts, identifiers or emails require review. Additional owners and unmapped information remain in the original document. A representative real partner application is still required to validate actual field coverage. Synthetic test coverage is not a claim that all application formats work.
...
Signature reuse is NOT implemented pending the user's answer about applicant authorization. The original document is unchanged. Removed the previous unconditional relabeling of any source as “Signed Application (executed)”. Generated Bypass forms are unsigned; the existing explicit signNow request action remains available. Original signature validity is not inferred by OCR.
