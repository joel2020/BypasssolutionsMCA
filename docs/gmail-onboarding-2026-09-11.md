...
The user subsequently confirmed `roman@bypasssolution.com` is the Google address. A recovered Google Cloud audience read still showed only Joel, proving the earlier save had not persisted. Re-entered the confirmed address and submitted Save, but the panel remained open and browser diagnostics repeatedly returned debugger-detached/command-timeout errors. Chrome was running; its extension was installed/enabled and native-host configuration checks passed. Opening the documented recovery window briefly restored reads, but subsequent save verification still failed. The enrollment form is left for manual completion; no successful enrollment or Roman mailbox connection is claimed.
