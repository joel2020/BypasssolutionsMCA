-- Self-enrollment is based only on the authenticated, verified Auth email.
...
begin
  ...
  where u.id = v_user_id
    ...
    and lower(u.email) ~ '^[^@[:space:]]+@bypasssolution[.]com$';
...
as $$
begin
  if coalesce(lower(event->'user'->>'email'), '') ~ '^[^@[:space:]]+@bypasssolution[.]com$' then
    return '{}'::jsonb;
  end if;
  return jsonb_build_object('error', jsonb_build_object(
    'http_code', 403,
    'message', 'New CRM accounts require a @bypasssolution.com email address.'
  ));
end;
$$;
