...
insert into public.leads (
  ...
  ('Bright Bistro LLC','Bright Bistro','Restaurant','LLC','2018-04-12',120000,1440000,75000,75000,18750,562300,3,245000,'Sarah','Johnson','Sarah Johnson','Managing Member','sarah@brightbistro.com','(212) 555-0178','New Lead','AT','Direct / Organic',now() - interval '2 hours',now() - interval '2 hours',true,'4321','1234','0021','7788','Owner requested same-week review.'),
  ('Urban Road Logistics','Urban Road','Transportation','Corporation','2016-09-02',410000,4920000,250000,250000,42500,905000,1,320000,'Mark','Spencer','Mark Spencer','President','mark@urbanroadlogistics.com','(718) 555-0144','Documents Needed','DP','Referral',now() - interval '1 day',now() - interval '3 hours',true,'1144','4411','9088','1255','Requested voided check and four months statements.'),
  ('Elite Auto Group','Elite Auto','Auto Sales','LLC','2013-03-19',500000,6000000,300000,300000,58700,1140000,0,125000,'David','Wilson','David Wilson','Owner','david@eliteautogroup.com','(201) 555-0199','Under Review','AT','Partner',now() - interval '1 day',now() - interval '1 hour',true,'0199','8901','6601','4400','Strong deposits, verify floor plan debt.'),
  ('NorthStar Restaurant','NorthStar','Restaurant','S-Corp','2017-11-06',320000,3840000,200000,200000,31750,790000,2,175000,'Olivia','Bennett','Olivia Bennett','CEO','olivia@northstarrestaurant.com','(646) 555-0125','Pre-Approved','AT','Direct / Organic',now() - interval '2 days',now() - interval '2 hours',true,'0125','3319','1822','7400','Pre-approval pending landlord verification.'),
  ...
  ('Coastal Plumbing Inc.','Coastal Plumbing','Contractor','Corporation','2015-05-30',250000,3000000,150000,150000,28750,610000,0,95000,'James','Carter','James Carter','President','james@coastalplumbing.com','(732) 555-0140','New Lead','DP','Referral',now() - interval '6 hours',now() - interval '6 hours',true,'0140','2188','9090','1120','Prefers morning calls.'),
  ('Sunshine Daycare','Sunshine Daycare','Childcare','LLC','2020-02-10',130000,1560000,80000,80000,16400,330000,2,45000,'Melissa','Green','Melissa Green','Owner','melissa@sunshinedaycare.com','(516) 555-0137','Documents Needed','KW','Direct / Organic',now() - interval '6 hours',now() - interval '6 hours',true,'0137','5501','8277','5300','Needs processing statements.'),
  ('GreenLeaf Landscaping','GreenLeaf','Landscaping','LLC','2018-08-08',180000,2160000,120000,120000,19800,430000,1,55000,'Chris','Martin','Chris Martin','Owner','chris@greenleaflandscaping.com','(908) 555-0170','Under Review','KW','Referral',now() - interval '2 days',now() - interval '5 hours',true,'0170','4320','3344','7122','Seasonal revenue; normalize trailing six months.'),
  ('FastLane Towing','FastLane','Automotive Services','LLC','2019-10-01',150000,1800000,95000,95000,14250,365000,4,70000,'Brian','Turner','Brian Turner','Owner','brian@fastlanetowing.com','(347) 555-0188','Pre-Approved','KW','Partner',now() - interval '3 days',now() - interval '6 hours',true,'0188','9182','4455','1880','Condition approval on NSF explanation.'),
  ...
  ('Peak Performance Gym','Peak Performance','Fitness','LLC','2020-01-01',95000,1140000,60000,60000,9700,220000,2,25000,'Rachel','Adams','Rachel Adams','Owner','rachel@peakperformancegym.com','(203) 555-0192','New Lead','AT','Direct / Organic',now() - interval '1 day',now() - interval '1 day',true,'0192','4001','6199','2221','Looking for equipment purchase funding.'),
  ('All Pro Construction','All Pro','Construction','Corporation','2012-04-04',350000,4200000,200000,200000,40200,820000,0,160000,'Tom','Reynolds','Tom Reynolds','President','tom@allproconstruction.com','(609) 555-0155','Documents Needed','AT','Partner',now() - interval '1 day',now() - interval '1 day',true,'0155','8820','6100','3144','Document requested: processing statements.'),
  ('Premium Staffing Co.','Premium Staffing','Staffing','LLC','2016-12-01',290000,3480000,175000,175000,33500,740000,0,115000,'Angela','Lee','Angela Lee','CEO','angela@premiumstaffing.com','(212) 555-0117','Under Review','DP','Referral',now() - interval '2 days',now() - interval '1 day',true,'0117','5098','2200','4511','Payroll lender payoff letter requested.'),
  ('Pro HVAC Services','Pro HVAC','HVAC','LLC','2017-09-09',190000,2280000,110000,110000,21500,485000,1,62000,'Kevin','White','Kevin White','Owner','kevin@prohvacservices.com','(856) 555-0168','Pre-Approved','DP','Direct / Organic',now() - interval '3 days',now() - interval '23 hours',true,'0168','3451','7200','4409','Eligible for split funding option.'),
...
alter table public.applications add constraint applications_status_check check (status in ('New Lead','Application Started','Documents Needed','Submitted','Under Review','Pre-Approved','Offer Sent','Approved','Contract Sent','Funded','Declined','Lost / No Response'));
...
select l.id, c.id, case when l.status = 'Docs Requested' then 'Documents Needed' else l.status end, l.source, l.funding_amount_requested, l.monthly_revenue, l.annual_revenue, l.avg_daily_balance, l.nsfs_last_90_days, l.submitted_at
...
from public.applications a join public.leads l on l.id = a.lead_id
where l.status in ('Documents Needed','New Lead')
on conflict do nothing;

insert into public.notes (lead_id, application_id, text, created_by_name)
select l.id, a.id, coalesce(l.notes, 'Initial CRM note added during seed import.'), 'Michael Anderson'
from public.applications a join public.leads l on l.id = a.lead_id
on conflict do nothing;
...
select l.id, a.id, uw.funding_partner, fp.id, l.funding_amount_requested, l.funding_amount_requested, uw.payback_amount, uw.factor_rate, uw.factor_rate, round(uw.payback_amount / 120), round(uw.payback_amount / 120), '120 business days', 'Daily', 'Daily', 6, round(l.funding_amount_requested * 0.06), case when l.status = 'Funded' then 'Accepted' when l.status = 'Offer Sent' then 'Sent' else 'Draft' end, 'Michael Anderson'
from public.applications a
join public.leads l on l.id = a.lead_id
...
where l.status in ('Offer Sent','Funded','Pre-Approved')
on conflict do nothing;
...
select a.id, fp.id, case when l.status in ('Offer Sent','Funded','Pre-Approved') then 'Submitted' else 'Prepared' end, case when l.status in ('Offer Sent','Funded','Pre-Approved') then now() - interval '1 day' else null end, 'Seeded partner submission record.'
from public.applications a
join public.leads l on l.id = a.lead_id
